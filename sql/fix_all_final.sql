...
-- ============================================
-- 1. Corrige funções com colunas corretas
-- 2. Dia ativo = Treino completo OU Foto na comunidade
-- 3. Refeições NÃO afetam streak

-- =====================================================
-- 1. FUNÇÃO DE WORKOUT TRACKING (CORRETA)
-- =====================================================
...
BEGIN
  ...
  -- community_posts SÓ existe se tiver foto (photo_url é obrigatório)
  SELECT EXISTS(
...
-- =====================================================
-- 2. FUNÇÃO DE MEAL TRACKING (CORRETA)
-- =====================================================
-- meal_tracking tem: cafe_da_manha, lanche_manha, almoco, lanche_tarde, janta, ceia

CREATE OR REPLACE FUNCTION sync_meal_tracking_to_daily_stats()
RETURNS TRIGGER AS $$
DECLARE
  today_date DATE := CURRENT_DATE;
  total_meals_planned INT := 6; -- Total de refeições possíveis
...
BEGIN
  ...
  -- Se não encontrou registro, considerar 0
  completed_meals_today := COALESCE(completed_meals_today, 0);

  -- Inserir ou atualizar daily_stats (NÃO TOCA em is_active_day)
  INSERT INTO daily_stats (
    aluno_id,
    date,
    meals_planned,
...
-- =====================================================
-- 4. RECALCULAR DADOS HISTÓRICOS
-- =====================================================

-- 4.1 Recalcular meals_completed com a lógica correta
DO $$
DECLARE
  meal_record RECORD;
  meals_count INT;
BEGIN
  FOR meal_record IN
    SELECT DISTINCT aluno_id, date FROM meal_tracking
  LOOP
    SELECT
      (CASE WHEN cafe_da_manha THEN 1 ELSE 0 END +
       CASE WHEN lanche_manha THEN 1 ELSE 0 END +
    ...
    INTO meals_count
    FROM meal_tracking
    WHERE aluno_id = meal_record.aluno_id
      AND date = meal_record.date;

    UPDATE daily_stats
    SET meals_completed = meals_count
    WHERE aluno_id = meal_record.aluno_id
      AND date = meal_record.date;
  END LOOP;

  RAISE NOTICE 'Recalculação de meals_completed concluída!';
END $$;

-- 4.2 Recalcular is_active_day com a lógica correta
DO $$
DECLARE
  ds_record RECORD;
  has_workout BOOLEAN;
...
BEGIN
  ...
  LOOP
    -- Verificar se tem treino completo
    SELECT EXISTS(
      SELECT 1 FROM workout_tracking
    ...
    -- Verificar se tem foto na comunidade
    SELECT EXISTS(
      SELECT 1 FROM community_posts
      WHERE aluno_id = ds_record.aluno_id
...
BEGIN
  ...
  LOOP
    ...
    -- Current streak só conta se o último dia ativo foi hoje ou ontem
...
SELECT
  ...
  us.total_meals_completed as refeicoes_completas,
