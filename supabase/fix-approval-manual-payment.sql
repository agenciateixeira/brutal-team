-- ============================================
-- FIX: Sistema de aprovação com pagamento manual
-- Aprova aluno = pagamento confirmado (sem payment_method)
-- ============================================

-- 1. REMOVER TRIGGERS ANTIGOS QUE USAVAM payment_method
-- ============================================
...
-- 2. CRIAR TABELA payment_history SE NÃO EXISTIR (sem payment_method)
-- ============================================
...
-- 3. CRIAR FUNÇÃO SIMPLIFICADA - Pagamento Manual
-- ============================================
...
BEGIN
  ...
  RAISE NOTICE 'Aluno % aprovado e ativado automaticamente - Plano: % - Valor: %',
    NEW.aluno_id,
    NEW.plan_type,
    NEW.monthly_value;
...
COMMENT ON FUNCTION auto_activate_student_on_plan_creation() IS
  'Ativa o aluno automaticamente quando plano é criado (pagamento manual - aprovação = pago)';

COMMENT ON TABLE public.payment_history IS
  'Histórico de pagamentos manuais dos alunos (sem payment_method)';

-- ✅ FIX COMPLETO!
--
-- O QUE FOI CORRIGIDO:
-- 1. ✅ Removido campo payment_method que causava erro
-- 2. ✅ Trigger simplificado para pagamento manual
-- 3. ✅ Aluno fica ATIVO imediatamente ao ser aprovado
-- 4. ✅ Registro de pagamento criado como confirmado
-- 5. ✅ Tabela payment_history criada/atualizada sem payment_method
--
-- NOVO FLUXO:
-- 1. Coach aprova aluno e define plano
-- 2. Trigger cria registro em payment_history (já confirmado)
-- 3. Perfil do aluno é atualizado com status 'active'
-- 4. student_plan marcado como payment_confirmed = true
-- 5. Aluno pode acessar o sistema imediatamente!
